...
int main(int argc, char **argv){
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &size);
        ...
        {
            # pragma omp single
                printf("task %d has spawned %d threads\n", rank, omp_get_num_threads() );
            int me = omp_get_thread_num();
            int mycore = sched_getcpu();
            printf("thread %2d of MPI task %d is running on core %2d\n", me, rank , mycore );
        }
        #endif
    #endif
...
double nBodyEvolution(particle *par, int rank, int size, int displacement, int offset, int n, double m){
    ...
    double tMin = 10;
    double *t; //W.r.t MPI we transform this to an array to avoid data races
    ...
    if((t = malloc(offset*sizeof(double))) == NULL){
        printf("Not enough memory to store the times array\n");
        exit(-1);
    }
